...
app.post('/staff-application', upload.single('cvUpload'), (req, res)=>{
    ...
    if(!fullName || !email || !phone || !address || !dob || !nationality || !formal_position || !formal_place_work || !startDate  || !responsibilities || !highestQualification || !institutionName || !graduationYear || !certifications || !teachingExperience || !subjects || !previousRoles || !coverLetter || !consent){
        ...
        try{
            ...
            const qry = "INSERT INTO staff_application(name, email, phone, address, dob, nationality, formal_position, formal_place_work, startDate, endDate, currentlyWorking, responsibilities, highestQualification,institutionName, graduationYear, certifications, teachingExperience, subjects, previousRoles, coverLetter,consent,cv, time_applied, date_applied)VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            con.query(qry, [fullName, email, phone, address, dob, nationality, formal_position, formal_place_work, startDate, endDate||'none', currentlyWorking || 'no', responsibilities, highestQualification,institutionName, graduationYear, certifications, teachingExperience, subjects, previousRoles, coverLetter,consent, cv, add, all], (err, result)=>{
                if(err){
                    console.log(err);
                }
                else{
...
app.post('/student-application', upload.single('passport'), (req, res)=>{
    ...
    const {student_name, dob, gender, nationality, apply_class, current_school, current_grade, reading_level, parent_name, parent_email,  parent_phone, parent_occupation,address, allergies, medical_conditions, medications, declaration, permission} = req.body;
    ...
    if(!student_name || !dob  || !gender || !nationality || !apply_class || !current_school || !current_grade || !reading_level || !parent_email || !parent_name || !parent_phone || !parent_occupation || !address || !declaration ||!permission){
        res.status(400).json();
    }
    else{
        try{
            ...
            const year = time.getFullYear();
            const all = `${year}-${month}-${day}`;
            const qry ="INSERT INTO student_application(passport, student_name, dob, gender, nationality, apply_class, current_school, current_grade, reading_level, parent_name, parent_phone, parent_occupation,address, allergies, medical_conditions, medications, declaration, permission, time_applied, date, parent_email)VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            con.query(qry, [passport,student_name, dob, gender, nationality, apply_class, current_school, current_grade, reading_level, parent_name, parent_phone, parent_occupation,address, allergies || 'none', medical_conditions || 'none', medications || 'none', declaration, permission, add, all, parent_email], (err, result)=>{
                if(err){
                    console.log(err);
                }
                else{
...
app.post('/student-reporters', (req, res)=>{
    try{
        const qry = "SELECT * FROM student_comment ORDER BY id DESC";
        con.query(qry, (err, result)=>{
            if(err){
                console.log(err);
            }
...
app.post('/staff-reporters', (req, res)=>{
    try{
        const qry = "SELECT * FROM staff_comment ORDER BY id DESC";
        con.query(qry, (err, result)=>{
            if(err){
                console.log(err);
            }
...
app.post('/student-application-display', (req, res)=>{
    try{
        const qry = "SELECT * FROM student_application";
        con.query(qry, (err, result)=>{
            if(err){
                console.log(err);
            }
...
app.post('/staff-application-display', (req, res)=>{
    try{
        const qry = "SELECT * FROM staff_application ORDER BY id DESC";
        con.query(qry, (err, result)=>{
            if(err){
                console.log(err);
            }
...
app.post('/commet-from-dashboard', (req, res)=>{
    try{
        const qry = "SELECT * FROM dashboard_commet ORDER BY id DESC";
        con.query(qry, (err, result)=>{
            if(err){
                console.log(err);
            }
...
app.post('/display-more-info-student-app', (req, res)=>{
    const {name, id} = req.body;
    // console.log(name, id);
    try{
        const qry = "SELECT * FROM student_application WHERE student_name=? AND id=?";
        con.query(qry, [name, id], (err, result)=>{
            if(err){
                console.log(err);
            }
            if(result.length>0){
...
app.post('/display-more-info-staff-app', (req, res)=>{
    const {name, id} = req.body;
    // console.log(name, id);
    try{
        const qry = "SELECT * FROM staff_application WHERE name=? AND id=?";
        con.query(qry, [name, id], (err, result)=>{
            if(err){
                console.log(err);
            }
...
//     const {name, title, email, subject, content} = req.body;
//      console.log(name, title, email, subject, content);
//    try {
//         const emailOptions = {
//             from:process.env.MAIL_FROM,
//             to:'ekundayojohn458@gmail.com',
//             subject:title,
//             text:content
//         }
...
app.post('/send-message', async (req, res) => {
    ...
    try {
        const emailOptions = {
            from: process.env.MAIL_FROM,
            to: 'sholadanielek@gmail.com',
            subject: subject || title || 'New Message',
            text: content,
            html: `
                <h2>New Message from ${name}</h2>
                <p><strong>Email:</strong> ${email}</p>
                <p><strong>Subject:</strong> ${subject || title || 'No subject'}</p>
                <p><strong>Message:</strong></p>
                <p>${content}</p>
            `
        };
        
        // 2. Send email and wait for result
